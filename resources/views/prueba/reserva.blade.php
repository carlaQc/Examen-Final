@extends('layouts.app')
@section('content')
<!--main content start-->
<section class="wrapper">
@include('layouts.newnotification')
{{-- Se verifica si el administrador del centro deportivo habilito el horario --}}
@if(!empty($field->first()->state))
<h3 class="text-center"><strong>Centro Deportivo - Horario "{{ $center->name_center }}"</strong></h3>
<h4 class="text-center"><i class="fa fa-calendar"></i> &nbsp;Horario Cancha <strong>{{ $name_field->name_field}}</strong></h4>
    


<div class="row">
    <div class="col-md-12">
        <div class="content-panel table-responsive formi">

            <table class="table table-striped table-advance table-hover">
        
            <h4><i class="fa fa-angle-right"></i> Canchas Disponibles:&nbsp;
                
                <div class="btn-group">
                    <button type="button" class="btn btn-theme03" style="background-color: #5bc0de; border-color: white;">{{ $name_field->name_field}}</button>
                    <button type="button" class="btn btn-theme03 dropdown-toggle" style="background-color: #5bc0de; border-color: white;" data-toggle="dropdown">
                        <span class="caret"></span>
                    </button>

                  <ul class="dropdown-menu" role="menu">
                    @foreach($field as $f)
                        <li>
                            <a href="{{ route('scheduleReserve.get',Crypt::encrypt($f->field_id)) }}">{{ $f->name_field}}</a>
                        </li>
                    @endforeach
                  </ul>
                </div>
                <a class="text-center">
                    <i class="fa fa-calendar"></i>&nbsp; Fecha Actual: {{ $current_date }}
                </a>
            </h4>
                <thead>
                    <hr>
                    <tr>
                        <th class="text-center"><i class="fa fa-clock-o"></i> Hora</th>
                        <th class="text-center">Lunes</th>
                        <th class="text-center">Martes</th>
                        <th class="text-center">Miercoles</th>
                        <th class="text-center">Jueves</th>
                        <th class="text-center">Viernes</th>
                        <th class="text-center">Sabado</th>
                        <th class="text-center">Domingo</th>
                    </tr>
                </thead>
                <tbody>
                    @php($start=0)
                    @php($end=7)
                    @php($sw=0)
    {{-- Se genera la tabla de Horario --}}
                    @foreach($hours as $hour)
                    <tr>
                        {{-- Muestra la hora --}}
                        <td class="text-center" style="vertical-align: inherit;"><strong>{{ $hour->hour }} - {{ $hour->hour_secondary }}</strong></td>
                        {{-- Se obtiene todos los datos de la fecha / para mostrar en el calendario --}}
                        @for($i=$start; $i<$end; $i++)

                        {{-- Arreglar el algoritmo de verificacion destado --}}
                        {{-- Verificar estado de hora --}}
                            {{-- {{ $reservation }} --}}
                            @if($sw==0)
                            @foreach($reservation as $r)
                                @if($r->hour == $date[$i]->number && $r->day == $date[$i]->day)
                                    {{-- Cuando el estado de la reserva ya fue aceptado o fue creado por el administrador --}}
                                    @if($r->reservation_state_id == 2)
                                    {{-- Modificar estilo del boton --}}
                                    @if(Auth::user()->rol_id == 2 || Auth::user()->rol_id == 3)
                                        <td class="text-center"> 
                                            <a style="text-decoration:none;float:center;" href="{{ route('scheduleStateReservation.edit',Crypt::encrypt($r->reservation_id)) }}"> 
                                                <button type="button" class="btn btn-success btn-sm">
                                                    RESERVADO
                                                </button> 
                                            </a>
                                        </td>
                                    @else
                                        <td class="text-center"> 
                                            <span disabled="true" type="button" class="btn btn-success btn-sm">
                                                @if ($r->state_payment_id == 3)
                                                    <i class="fa fa-check"></i>&nbsp;RESERVADO</span>
                                                @else
                                                    <i class="fa fa-thumb-tack"></i>&nbsp;RESERVADO</span>
                                                @endif
                                                
                                        </td>
                                    @endif
                                    {{-- fin Modificar estilo del boton --}}
                                    @php($sw=1)
                                    @endif
                                @endif
                            @endforeach
                            @endif

                            {{-- Cuando el administrador del centro deportivo inhabilita el dia de reserva --}}
                            @if($sw == 0 && $date[$i]->state_day == 0)
                            {{-- Modificar estilo del boton --}}
                            <td class="text-center"> 
                                <span disabled="true" type="button" class="btn btn-default btn-sm">INHABILITADO</span>
                            </td>
                            {{-- fin Modificar estilo del boton --}}
                            @php($sw=1)
                            @endif

                            {{-- Cuando el adminstrador inhabilita una hora --}}
                            @if($sw == 0 && $date[$i]->state_hour == 0)
                            {{-- Modificar estilo del boton --}}
                            <td class="text-center"> 
                                <span disabled="true" type="button" class="btn btn-default btn-sm">INHABILITADO</span>
                            </td>
                            @php($sw=1)
                            {{-- fin Modificar estilo del boton --}}
                            @endif

                            {{-- Cuando el dia ya paso se inhabilita el boton --}}
                            @if($sw == 0 && $date[$i]->day < $day)
                            {{-- Modificar estilo del boton --}}
                            <td class="text-center"> 
                                <span disabled="true" type="button" class="btn btn-default btn-sm">INHABILITADO</span>
                                {{-- <button disabled="true" type="button" class="btn btn-default">
                                    INHABILITADO
                                </button> --}} 
                            </td>
                            @php($sw=1)
                            {{-- fin Modificar estilo del boton --}}
                            @endif

                            {{-- El horario se acopla al horario actual/se coordina la hora actual --}}
                            {{-- la hora y el dia del calendario son menores a la hora actual se restringe la reserva y se inhabilita el boton --}}
                            @if($sw == 0 && $date[$i]->day == $day && $date[$i]->number < $h)
                            {{-- Modificar estilo del boton --}}
                                <td class="text-center"> 
                                    <span type="button" class="btn btn-default btn-sm">INHABILITADO</span>
                                    {{-- <button disabled="true" type="button" class="label label-default">
                                        INHABILITADO
                                    </button> --}} 
                                </td>
                            {{-- fin Modificar estilo del boton --}}
                            @php($sw=1)
                            @endif

                            {{-- verificacion de reserva en el estado --}}
                            @if($sw==0)
                            @foreach($reservation as $r)
                                @if($r->hour == $date[$i]->number && $r->day == $date[$i]->day)

                                    @if($r->reservation_state_id == 1)
                                    {{-- Estado pendiente / cuando es creado por un cliente y no poe el administrador del centro deportivo --}}
                                    {{-- Modificar estilo del boton --}}

                                    @if(Auth::user()->rol_id == 2 || Auth::user()->rol_id == 3)
                                    {{-- Crypt::encrypt($r->reservation_id) Encripta los datos     --}}
                                        <td class="text-center"> 
                                            <a style="text-decoration:none;float:center;" href="{{ route('scheduleStateReservation.edit',Crypt::encrypt($r->reservation_id)) }}"> 
                                                <button type="button" class="btn btn-warning btn-sm">
                                                    PENDIENTE
                                                </button>
                                            </a>
                                        </td>
                                    @endif
                                    @if(Auth::user()->rol_id == 1 || Auth::user()->rol_id == 4)
                                        <td class="text-center"> 
                                            {{-- <a style="text-decoration:none;float:center;" href="{{ route('scheduleStateReservation.edit',$data[$i]->center_id) }}"> --}}
                                                <span type="button" class="btn btn-warning btn-sm"><i class="fa fa-thumb-tack"></i>&nbsp;PENDIENTE</span>
                                                {{-- <button type="button" class="label label-warning">
                                                    PENDIENTE
                                                </button>  --}}
                                            {{-- </a> --}}
                                        </td>
                                    @endif

                                    {{-- fin Modificar estilo del boton --}}
                                    @php($sw=1)  
                                    @endif
                                @endif
                            @endforeach
                            @endif
                            {{-- FIn de verificacion de estado --}}

                            {{-- si el horario esta libre o expiro alguna reserva puede hacer una nueva reserva --}}
                            @if($sw == 0)
                            {{-- en route se envia: center_id, day, number(envia la hora) --}}
                            {{-- Modificar estilo del boton --}}
                                <td class="text-center"> 
                                    <a style="text-decoration:none;float:center;" href="{{ route('scheduleReserve.create',[Crypt::encrypt($date[$i]->field_id), Crypt::encrypt($date[$i]->day), Crypt::encrypt($date[$i]->number)]) }}">
                                        <button type="button" class="btn btn-primary btn-sm">
                                            HABILITADO
                                        </button>
                                    </a>
                                </td>
                            {{-- fin Modificar estilo del boton --}}  
                            @endif    
                            @php($sw=0)  
                            {{-- Fin de modulo de reserva --}}
                        
                        @php($start = $i+1)
                        @endfor
                        @php($end = $end+7)
                    </tr>
                    @endforeach
    {{-- Fin de la tabla de Horario --}}
                </tbody>
            </table>
        </div>
    </div><!-- /col-md-12 -->
</div><!-- row -->

@else
    <h3 class="text-center"><strong>Horario "{{ $center->name_center }}" - No Disponible </strong></h3>
@endif
</section>
@endsection
